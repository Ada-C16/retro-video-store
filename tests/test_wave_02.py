...
def test_checkout_video_not_found(client, one_video, one_customer):
    ...
    response = client.post("/rentals/check-out", json={"customer_id": 1, "video_id": 2})
...
def test_checkout_customer_video_not_found(client, one_video, one_customer):
    ...
    response = client.post("/rentals/check-out", json={"customer_id": 2, "video_id": 1})
...
def test_checkin_video(client, one_checked_out_video):
    response = client.post("/rentals/check-in", json={"customer_id": 1, "video_id": 1})
...
def test_checkin_video_no_customer_id(client, one_checked_out_video):
    response = client.post("/rentals/check-in", json={"video_id": 1})

    assert response.status_code == 400


def test_checkin_video_no_video_id(client, one_checked_out_video):
    response = client.post("/rentals/check-in", json={"customer_id": 1})

    assert response.status_code == 400


def test_checkin_video_not_found(client, one_checked_out_video):
    response = client.post("/rentals/check-in", json={"customer_id": 2, "video_id": 1})

    assert response.status_code == 404


def test_checkin_customer_not_found(client, one_checked_out_video):
    response = client.post("/rentals/check-in", json={"customer_id": 1, "video_id": 2})
