...
def test_create_video_must_contain_title(client):
    ...
    response = client.post(
        "/videos",
        json={"release_date": VIDEO_RELEASE_DATE, "total_inventory": VIDEO_INVENTORY},
    )
    response_body = response.get_json()
...
def test_create_video_must_contain_release_date(client):
    # Act
    response = client.post(
        "/videos", json={"title": VIDEO_TITLE, "total_inventory": VIDEO_INVENTORY}
    )
    response_body = response.get_json()
...
def test_create_video_must_contain_inventory(client):
    # Act
    response = client.post(
        "/videos", json={"title": VIDEO_TITLE, "release_date": VIDEO_RELEASE_DATE}
    )
    response_body = response.get_json()
...
def test_update_video_invalid_data(client, one_video):
    # Act
    response = client.put(
        "/videos/1", json={"total_inventory": 2, "release_date": "01-01-2021"}
    )
...
def test_create_customer_must_contain_name(client):
    ...
    response = client.post(
        "/customers",
        json={"phone": CUSTOMER_PHONE, "postal_code": CUSTOMER_POSTAL_CODE},
    )
    response_body = response.get_json()
...
def test_create_customer_must_contain_phone(client):
    # Act
    response = client.post(
        "/customers", json={"name": CUSTOMER_NAME, "postal_code": CUSTOMER_POSTAL_CODE}
    )
    response_body = response.get_json()
